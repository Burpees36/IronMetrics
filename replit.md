...
- **Testing:** Vitest with 233 tests across 15 test files, all passing. Backend (211 tests in 13 files): billing metrics pure functions, billing recovery integration, email builders, recovery config, token generation/validation, RBAC permission matrices (billing, schedule, programming), requireGymAccess middleware, intelligence computations (RSI scoring, risk tiers), and recommendation learning (classification, exponential smoothing). Frontend (22 tests in 2 files): Dashboard component (KPI rendering, loading states, RSI badge, at-risk alert), UpdatePayment page. Coverage scripts: `pnpm --filter @workspace/api-server run test:coverage`, `pnpm --filter @workspace/iron-metrics run test:coverage`.
