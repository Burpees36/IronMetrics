...
-   **Error & Empty States:** Reusable `PageError` component (`page-error.tsx`) with retry button, used across Dashboard, Members, Billing, Workouts, Leads, and Retention pages. Each page checks for API errors (`isError`/`loadError`) and shows `PageError` with a contextual message and retry callback. Loading states always render before error checks to avoid premature false errors.
