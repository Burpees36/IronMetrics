...
- **Testing:** Vitest with 303 tests across 20 test files, all passing. Backend (263 tests in 16 files): webhook handlers (subscription lifecycle, payment success/failure, refunds, idempotency via claimEvent), AI task generation (at-risk outreach, onboarding, stale leads, billing recovery, duplicate prevention), member import validation (normalizePhone, isValidCalendarDate, parseImportDate, sanitizeRow, validateRow), billing metrics, billing recovery integration, email builders, recovery config, token generation/validation, RBAC permission matrices (billing, schedule, programming), requireGymAccess middleware, intelligence computations (RSI scoring, risk tiers), recommendation learning (classification, exponential smoothing). Frontend (40 tests in 4 files): Dashboard, Intelligence (RSI display, error states, tab navigation), Members (listing, search, status badges, risk tiers), UpdatePayment. Coverage: `@vitest/coverage-v8` installed; config includes routes, services, middlewares, schedulers, webhookHandlers. Scripts: `pnpm --filter @workspace/api-server run test:coverage`, `pnpm --filter @workspace/iron-metrics run test:coverage`.
