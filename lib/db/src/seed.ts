...
async function seed() {
  ...
  await db.insert(aiTasksTable).values([
    { gymId: gym.id, type: "outreach", title: `Re-engage ${arm0.firstName}`, description: `${arm0.firstName} ${arm0.lastName} has only visited ${arm0.attendanceCount30d} time(s) in the last 30 days. Schedule a goal review or casual catch-up.`, priority: "high", status: "pending", targetId: arm0.id, targetType: "member", aiContent: `Hi ${arm0.firstName},\n\nJust wanted to reach out and see how things are going! It's been a little while since we've seen you, and we genuinely miss having you around.\n\nI'd love to schedule a quick goal review — even just 10 minutes to check in on your progress and make sure we're helping you hit your targets. We also have some great upcoming events and challenges that might be right up your alley.\n\nWant to grab a quick coffee or chat at the gym this week? Let me know what works for you!` },
    { gymId: gym.id, type: "outreach", title: `Win back ${arm1.firstName}`, description: `${arm1.firstName} ${arm1.lastName} shows critical churn risk. Reach out with a personal invitation to reconnect.`, priority: "high", status: "pending", targetId: arm1.id, targetType: "member", aiContent: `Hi ${arm1.firstName},\n\nI was thinking about you and wanted to reach out personally. We've got some exciting new programming and challenges coming up that I think you'd really enjoy.\n\nI'd love to set up a quick goal review session — just 15 minutes to catch up, see where you're at, and map out a plan that fits your schedule. No pressure at all, just a chance to reconnect.\n\nWould you be free for a coffee or a quick chat at the gym this week? I'll buy the coffee.\n\nLooking forward to hearing from you!` },
    { gymId: gym.id, type: "outreach", title: `Check in with ${arm2.firstName}`, description: `${arm2.firstName} has been flagged as high risk. A quick personal touchpoint could help retain them.`, priority: "medium", status: "pending", targetId: arm2.id, targetType: "member", aiContent: `Hi ${arm2.firstName},\n\nJust wanted to check in and see how everything's going. We've got a couple of great challenges and events coming up soon, and I thought you might be interested.\n\nHow about we grab a quick coffee or schedule a 10-minute goal review? I'd love to hear what's on your mind and see how we can help.\n\nHope to see you soon!` },
    { gymId: gym.id, type: "retention", title: `Welcome check-in: ${nm0.firstName}`, description: `${nm0.firstName} ${nm0.lastName} joined recently. Schedule an intro check-in to set goals and ensure a smooth start.`, priority: "medium", status: "pending", targetId: nm0.id, targetType: "member", aiContent: `Hi ${nm0.firstName},\n\nWelcome to the family! We're so glad you're here.\n\nI'd love to schedule a quick intro check-in — just 15 minutes to talk about your goals, answer any questions, and make sure you feel comfortable.\n\nWe'll get you set up with a plan that fits your schedule and goals. Every expert was once a beginner, and we're here for you every step of the way.\n\nLooking forward to chatting!\n\nYour coaching team` },
    { gymId: gym.id, type: "retention", title: `Day 14 check-in: ${nm1.firstName}`, description: `${nm1.firstName} is in their first 90 days. Schedule a progress check-in to boost retention.`, priority: "low", status: "pending", targetId: nm1.id, targetType: "member", aiContent: `Hi ${nm1.firstName},\n\nYou're making great progress! Let's schedule a quick 10-minute check-in to review your goals and make sure you're getting the most out of your membership.\n\nWe'll look at your attendance, talk about any movements you want to work on, and set some fun milestones for the next month. These check-ins are what help our members see real results.` },
    { gymId: gym.id, type: "leads", title: `Schedule No Sweat Intro: ${staleLead!.firstName} ${staleLead!.lastName}`, description: `${staleLead!.firstName} ${staleLead!.lastName} was contacted via ${staleLead!.source} but hasn't booked yet. Invite them to a No Sweat Intro.`, priority: "medium", status: "pending", targetId: staleLead!.id, targetType: "lead", aiContent: `Hi ${staleLead!.firstName},\n\nI wanted to follow up and see if you're still interested in checking us out!\n\nWe'd love to have you in for a No Sweat Intro — it's a free, no-pressure consultation where we sit down, learn about your goals, and show you around the gym. It takes about 20 minutes and there's zero obligation.\n\nWould any of these times work for you this week?` },
    { gymId: gym.id, type: "leads", title: `Book No Sweat Intro: ${trialLead!.firstName} ${trialLead!.lastName}`, description: `${trialLead!.firstName} ${trialLead!.lastName} showed interest. Follow up to schedule a No Sweat Intro and convert to membership.`, priority: "high", status: "pending", targetId: trialLead!.id, targetType: "lead", aiContent: `Hi ${trialLead!.firstName},\n\nGreat chatting with you! I'd love to get you set up with a No Sweat Intro — it's a free, relaxed sit-down where we talk about your goals, show you the space, and figure out the best path for you.\n\nNo workout required, no pressure. Just a conversation to see if we're a good fit.\n\nWhat day this week works best for you?` },
    { gymId: gym.id, type: "billing", title: `Follow up on hold member: ${holdMem.firstName} ${holdMem.lastName}`, description: `${holdMem.firstName} ${holdMem.lastName} has been on hold. Check in warmly to see if they're ready to return.`, priority: "medium", status: "pending", targetId: holdMem.id, targetType: "member", aiContent: `Hi ${holdMem.firstName},\n\nHope you're doing well! I noticed your membership is currently on hold, and I just wanted to check in.\n\nThere's no rush at all — whenever you're ready, we'd love to have you back. We've got some great new programming and a few fun events coming up that I think you'd enjoy.\n\nIf there's anything we can do to make your return easier, just let me know. We're here for you!` },
    { gymId: gym.id, type: "campaign", title: "Launch spring referral campaign", description: "Design a member referral campaign offering a free month for each successful referral to boost growth.", priority: "low", status: "pending", aiContent: `Spring Referral Program:\n- Members get 1 free month for each referral who signs up\n- New members get 10% off first 3 months\n- Campaign runs April 1-30\n- Promote via email blast + in-gym signage + Bright Spots Friday shout-outs` },
