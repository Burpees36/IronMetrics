              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[10px] text-muted-foreground flex items-center gap-1">
                    ...
                    {i === 0 ? `After ${formatDelayLabel(step.delayMinutes)}` : `+${formatDelayLabel(step.delayMinutes)}`}
                  </span>
                </div>
                {step.subject && (
                  <p className="text-xs font-medium text-foreground mb-1">{step.subject}</p>
                )}
                <p className="text-xs text-muted-foreground line-clamp-2">{step.messageContent}</p>
              </div>
            </div>
          ))}
        </div>
            ...
            <div key={i} className="bg-muted/20 border border-border rounded-xl p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-medium text-foreground">Step {i + 1}</span>
                </div>
                {steps.length > 1 && (
                  <button onClick={() => removeStep(i)} className="p-1 rounded hover:bg-red-500/10 text-muted-foreground hover:text-red-500 transition-colors">
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div>
                  <label className="text-[10px] font-medium text-muted-foreground mb-1 block">Channel</label>
                  <select
                    value={step.channel}
                ...
                <div>
                  <label className="text-[10px] font-medium text-muted-foreground mb-1 block">Delay</label>
                  <select
              ...
              {step.channel === "email" && (
                <div className="mb-3">
                  <label className="text-[10px] font-medium text-muted-foreground mb-1 block">Subject Line</label>
                  <input
                    type="text"
                    value={step.subject || ""}
